...
<Steps>
  ...
  <Step title="Battle Log">
    ...
    Each line is wrapped in a `motion.p` and animates from `opacity: 0` to `opacity: 1` at a staggered delay:

    | Line | Delay |
    |------|-------|
    | Battle initiated… | 0.5 s |
    | Player uses MICROSERVICES | 1.0 s |
    | Boss attacks with SPAGHETTI CODE | 1.5 s |
    | Player casts TYPESCRIPT MIGRATION | 2.0 s |
    | Boss defeated. | 2.5 s |
  </Step>
